...
public class WeightedJobSchedulingMaximumProfit 
{
	...
	static void quickSort(Job[] jobs, int first, int last)
	{
		Job temp;
		...
		if (first < last)
		{
			...
			while (i < j)
			{
				while ( jobs[i].profit <= jobs[pivot].profit && i < last)
				{
					i++;
				}
				while (jobs[i].profit  > jobs[pivot].profit)
				{
					j--;
				}
				if (i < j)
			...
			quickSort(jobs, first, j - 1);
			quickSort(jobs, j + 1, last);
		}
	}

	static void printResult(Job[] jobs)
	{
...
//        jobs[5] = new Job(7,9,2);
	    ...
	    jobs[0] = new Job(1, 5, 30);
	    jobs[1] = new Job(4, 7, 25);
	    jobs[2] = new Job(2, 9, 42);
	    jobs[3] = new Job(8, 10, 9);
	    jobs[4] = new Job(1, 4, 20);
	    jobs[5] = new Job(5, 9, 27);
	    jobs[6] = new Job(7, 10, 16);
	    jobs[7] = new Job(1, 3, 15);
	    jobs[8] = new Job(5, 7, 7);
